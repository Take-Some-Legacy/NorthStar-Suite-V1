...
set "EXE=%TOOL_ROOT%\bin\cp.exe"
set "WORK=%TOOL_ROOT%\test\_out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.cp functional smokeTest
...
"%EXE%" "%WORK%\source.txt" "%WORK%\copy.txt"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
fc "%WORK%\source.txt" "%WORK%\copy.txt" > nul || set "RC=1"
if not "%RC%"=="0" goto :done
echo [PASS] cp copied file and content matched
