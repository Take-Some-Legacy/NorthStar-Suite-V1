...
set "EXE=%TOOL_ROOT%\bin\find.exe"
set "OUT=%TEMP%\northstar-find-smoke.out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.find smokeTest
...
"%EXE%" . -maxdepth 0 -type d > "%OUT%"
set "RC=%ERRORLEVEL%"
type "%OUT%"
if not "%RC%"=="0" goto :done
for %%I in ("%OUT%") do if %%~zI LEQ 0 set "RC=1"
if not "%RC%"=="0" goto :done
echo [PASS] vendor.msys2.gnu.find smokeTest passed
:done
