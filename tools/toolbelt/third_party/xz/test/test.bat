...
set "EXE=%TOOL_ROOT%\bin\xz.exe"
set "WORK=%TOOL_ROOT%\test\_out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.xz functional smokeTest
...
echo xz-smoke> "%WORK%\input.txt"
"%EXE%" -k "%WORK%\input.txt"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
if not exist "%WORK%\input.txt.xz" echo [FAIL] xz output missing& set "RC=1"& goto :done
"%EXE%" -dc "%WORK%\input.txt.xz" > "%WORK%\roundtrip.txt"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
findstr /x /c:"xz-smoke" "%WORK%\roundtrip.txt" > nul || set "RC=1"
if not "%RC%"=="0" goto :done
echo [PASS] xz compressed and decompressed temp file
