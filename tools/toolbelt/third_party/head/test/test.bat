...
set "EXE=%TOOL_ROOT%\bin\head.exe"
set "WORK=%TOOL_ROOT%\test\_out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.head functional smokeTest
...
(echo one& echo two& echo three)> "%WORK%\input.txt"
"%EXE%" -n 2 "%WORK%\input.txt" > "%WORK%\out.txt"
set "RC=%ERRORLEVEL%"
type "%WORK%\out.txt"
if not "%RC%"=="0" goto :done
findstr /x /c:"one" "%WORK%\out.txt" > nul || set "RC=1"
findstr /x /c:"two" "%WORK%\out.txt" > nul || set "RC=1"
findstr /x /c:"three" "%WORK%\out.txt" > nul && set "RC=1"
if not "%RC%"=="0" goto :done
echo [PASS] head returned first two lines only
