...
Third-party GNUWin32 `sed.exe` package.

This package is self-contained: executable payload and runtime DLL dependencies live inside this package's `bin/` directory.

```text
package root: tools/toolbelt/third_party/sed
bin:          tools/toolbelt/third_party/sed/bin/
entrypoint:   tools/toolbelt/third_party/sed/bin/sed.exe
```
...
- Do not call `sed.exe` directly from reusable engine/runtime code.
