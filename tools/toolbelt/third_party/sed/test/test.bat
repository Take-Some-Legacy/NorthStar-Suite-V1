...
set "EXE=%TOOL_ROOT%\bin\sed.exe"
set "DATA=%TOOL_ROOT%\test\testData"
set "OUT=%TEMP%\northstar-msys2-sed-smoke.out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.sed smokeTest
...
echo [CMD] "%EXE%" -e s/foo/bar/g "%DATA%\input.txt"
"%EXE%" -e s/foo/bar/g "%DATA%\input.txt" > "%OUT%"
set "RC=%ERRORLEVEL%"
type "%OUT%"
...
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
echo [PASS] vendor.msys2.gnu.sed smokeTest passed
goto :done
:done
