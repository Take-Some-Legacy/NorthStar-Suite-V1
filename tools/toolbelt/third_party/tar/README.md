...
Third-party GNUWin32 `tar.exe` package.

This package is self-contained: executable payload and runtime DLL dependencies live inside this package's `bin/` directory.

```text
package root: tools/toolbelt/third_party/tar
bin:          tools/toolbelt/third_party/tar/bin/
entrypoint:   tools/toolbelt/third_party/tar/bin/tar.exe
```
...
- Do not call `tar.exe` directly from reusable engine/runtime code.
