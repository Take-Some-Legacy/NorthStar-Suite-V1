...
set "EXE=%TOOL_ROOT%\bin\mv.exe"
set "WORK=%TOOL_ROOT%\test\_out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.mv functional smokeTest
...
echo alpha> "%WORK%\source.txt"
"%EXE%" "%WORK%\source.txt" "%WORK%\moved.txt"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
if exist "%WORK%\source.txt" echo [FAIL] source still exists& set "RC=1"& goto :done
findstr /c:"alpha" "%WORK%\moved.txt" > nul || set "RC=1"
if not "%RC%"=="0" goto :done
echo [PASS] mv moved file and preserved content
