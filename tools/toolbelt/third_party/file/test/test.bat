...
set "EXE=%TOOL_ROOT%\bin\file.exe"
set "OUT=%TEMP%\northstar-file-smoke.out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.file smokeTest
...
set "MAGIC=%TOOL_ROOT%\share\misc\magic.mgc"
"%EXE%" "%TOOL_ROOT%\tool.json" > "%OUT%"
set "RC=%ERRORLEVEL%"
type "%OUT%"
if not "%RC%"=="0" goto :done
for %%I in ("%OUT%") do if %%~zI LEQ 0 set "RC=1"
if not "%RC%"=="0" goto :done
echo [PASS] vendor.msys2.gnu.file smokeTest passed
:done
