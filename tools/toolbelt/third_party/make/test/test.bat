...
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.make functional smokeTest
...
set "MK=%WORK%\Makefile"
> "%MK%" echo RESULT := NorthStar
>> "%MK%" echo all:
>> "%MK%" echo ^	@echo $(RESULT^) Smoke
"%EXE%" -n -f "%MK%" all > "%WORK%\out.txt"
set "RC=%ERRORLEVEL%"
type "%WORK%\out.txt"
if not "%RC%"=="0" goto :done
findstr /c:"NorthStar Smoke" "%WORK%\out.txt" > nul || set "RC=1"
if not "%RC%"=="0" goto :done
echo [PASS] make parsed makefile and expanded recipe in dry-run
