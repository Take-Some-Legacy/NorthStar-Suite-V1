...
set "EXE=%TOOL_ROOT%\bin\realpath.exe"
set "WORK=%TOOL_ROOT%\test\_out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.realpath functional smokeTest
...
echo alpha> "%WORK%\target.txt"
"%EXE%" "%WORK%\target.txt" > "%WORK%\out.txt"
set "RC=%ERRORLEVEL%"
type "%WORK%\out.txt"
if not "%RC%"=="0" goto :done
findstr /i /c:"target.txt" "%WORK%\out.txt" > nul || set "RC=1"
if not "%RC%"=="0" goto :done
echo [PASS] realpath resolved temp file
