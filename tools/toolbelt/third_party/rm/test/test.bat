...
set "EXE=%TOOL_ROOT%\bin\rm.exe"
set "WORK=%TOOL_ROOT%\test\_out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.rm functional smokeTest
...
echo alpha> "%WORK%\delete-me.txt"
if not exist "%WORK%\delete-me.txt" echo [FAIL] fixture create failed& set "RC=1"& goto :done
"%EXE%" "%WORK%\delete-me.txt"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
if exist "%WORK%\delete-me.txt" echo [FAIL] rm did not delete file& set "RC=1"& goto :done
echo [PASS] rm deleted temp file
