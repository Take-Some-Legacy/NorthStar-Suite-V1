...
set "EXE=%TOOL_ROOT%\bin\sort.exe"
set "DATA=%TOOL_ROOT%\test\testData"
set "OUT=%TEMP%\northstar-msys2-sort-smoke.out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.sort smokeTest
...
"%EXE%" --version > "%TEMP%\northstar-msys2-sort-version.out"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
type "%TEMP%\northstar-msys2-sort-version.out" | findstr /c:"GNU coreutils" > nul
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
echo [CMD] tool-specific smoke
"%EXE%" "%DATA%\input.txt" > "%OUT%"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
type "%OUT%"
fc "%OUT%" "%DATA%\expected-sort.txt" > nul
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
echo [PASS] vendor.msys2.gnu.sort smokeTest passed
goto :done
:done
