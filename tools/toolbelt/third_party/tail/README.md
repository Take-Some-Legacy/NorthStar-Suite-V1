...
Third-party GNUWin32 `tail.exe` package.

This package is self-contained: executable payload and runtime DLL dependencies live inside this package's `bin/` directory.

```text
package root: tools/toolbelt/third_party/tail
bin:          tools/toolbelt/third_party/tail/bin/
entrypoint:   tools/toolbelt/third_party/tail/bin/tail.exe
```
...
- Do not call `tail.exe` directly from reusable engine/runtime code.
