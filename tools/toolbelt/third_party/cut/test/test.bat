...
set "EXE=%TOOL_ROOT%\bin\cut.exe"
set "WORK=%TOOL_ROOT%\test\_out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.cut functional smokeTest
...
echo one,two,three> "%WORK%\input.csv"
"%EXE%" -d, -f2 "%WORK%\input.csv" > "%WORK%\out.txt"
set "RC=%ERRORLEVEL%"
type "%WORK%\out.txt"
if not "%RC%"=="0" goto :done
py -c "from pathlib import Path; raise SystemExit(0 if Path(r'%WORK%\out.txt').read_text().strip()=='two' else 1)" || set "RC=1"
if not "%RC%"=="0" goto :done
echo [PASS] cut extracted second CSV field
