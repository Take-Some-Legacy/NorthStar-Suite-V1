...
set "EXE=%TOOL_ROOT%\bin\printf.exe"
set "WORK=%TOOL_ROOT%\test\_out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.printf functional smokeTest
...
py -c "from pathlib import Path; raise SystemExit(0 if Path(r'%WORK%\out.txt').read_text().strip()=='NorthStar-007' else 1)" || set "RC=1"
if not "%RC%"=="0" goto :done
echo [PASS] printf formatted output
