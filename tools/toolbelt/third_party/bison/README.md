...
Third-party GNUWin32 `bison.exe` package.

This package is self-contained: executable payload and runtime DLL dependencies live inside this package's `bin/` directory.

```text
package root: tools/toolbelt/third_party/bison
bin:          tools/toolbelt/third_party/bison/bin/
entrypoint:   tools/toolbelt/third_party/bison/bin/bison.exe
```
...
- Do not call `bison.exe` directly from reusable engine/runtime code.
