...
set "EXE=%TOOL_ROOT%\bin\wc.exe"
set "DATA=%TOOL_ROOT%\test\testData"
set "OUT=%TEMP%\northstar-msys2-wc-smoke.out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.wc smokeTest
...
"%EXE%" --version > "%TEMP%\northstar-msys2-wc-version.out"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
type "%TEMP%\northstar-msys2-wc-version.out" | findstr /c:"GNU coreutils" > nul
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
echo [CMD] tool-specific smoke
"%EXE%" -l "%DATA%\input.txt" > "%OUT%"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
type "%OUT%"
findstr /b /c:"4 " "%OUT%" > nul
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
echo [PASS] vendor.msys2.gnu.wc smokeTest passed
goto :done
:done
