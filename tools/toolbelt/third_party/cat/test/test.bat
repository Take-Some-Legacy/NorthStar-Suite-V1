...
set "EXE=%TOOL_ROOT%\bin\cat.exe"
set "DATA=%TOOL_ROOT%\test\testData"
set "OUT=%TEMP%\northstar-msys2-cat-smoke.out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.cat smokeTest
...
"%EXE%" --version > "%TEMP%\northstar-msys2-cat-version.out"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
type "%TEMP%\northstar-msys2-cat-version.out" | findstr /c:"GNU coreutils" > nul
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
...
if not "%RC%"=="0" goto :done
type "%OUT%"
fc "%OUT%" "%DATA%\expected-cat.txt" > nul
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
echo [PASS] vendor.msys2.gnu.cat smokeTest passed
goto :done
:done
