...
set "EXE=%TOOL_ROOT%\bin\basename.exe"
set "WORK=%TOOL_ROOT%\test\_out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.basename functional smokeTest
...
py -c "from pathlib import Path; raise SystemExit(0 if Path(r'%WORK%\out.txt').read_text().strip()=='file.txt' else 1)" || set "RC=1"
if not "%RC%"=="0" goto :done
echo [PASS] basename extracted file name
