...
set "EXE=%TOOL_ROOT%\bin\uniq.exe"
set "DATA=%TOOL_ROOT%\test\testData"
set "OUT=%TEMP%\northstar-msys2-uniq-smoke.out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.uniq smokeTest
...
"%EXE%" --version > "%TEMP%\northstar-msys2-uniq-version.out"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
type "%TEMP%\northstar-msys2-uniq-version.out" | findstr /c:"GNU coreutils" > nul
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
...
if not "%RC%"=="0" goto :done
type "%OUT%"
fc "%OUT%" "%DATA%\expected-uniq.txt" > nul
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
echo [PASS] vendor.msys2.gnu.uniq smokeTest passed
goto :done
:done
