...
set "EXE=%TOOL_ROOT%\bin\gzip.exe"
set "WORK=%TOOL_ROOT%\test\_out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.gzip functional smokeTest
...
echo gzip-smoke> "%WORK%\input.txt"
"%EXE%" -k "%WORK%\input.txt"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
if not exist "%WORK%\input.txt.gz" echo [FAIL] gzip output missing& set "RC=1"& goto :done
"%EXE%" -dc "%WORK%\input.txt.gz" > "%WORK%\roundtrip.txt"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
findstr /x /c:"gzip-smoke" "%WORK%\roundtrip.txt" > nul || set "RC=1"
if not "%RC%"=="0" goto :done
echo [PASS] gzip compressed and decompressed temp file
