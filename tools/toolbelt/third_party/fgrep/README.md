...
Third-party GNUWin32 `fgrep.exe` package.

This package is self-contained: executable payload and runtime DLL dependencies live inside this package's `bin/` directory.

```text
package root: tools/toolbelt/third_party/fgrep
bin:          tools/toolbelt/third_party/fgrep/bin/
entrypoint:   tools/toolbelt/third_party/fgrep/bin/fgrep.exe
```
...
- Do not call `fgrep.exe` directly from reusable engine/runtime code.
