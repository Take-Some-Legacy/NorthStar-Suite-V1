...
set "EXE=%TOOL_ROOT%\bin\ls.exe"
set "WORK=%TOOL_ROOT%\test\_out"
set "RC=0"
echo ============================================================
echo [SMOKE] vendor.msys2.gnu.ls functional smokeTest
...
echo beta> "%WORK%\beta.txt"
"%EXE%" -1 "%WORK%" > "%WORK%\ls.out"
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" goto :done
type "%WORK%\ls.out"
findstr /c:"alpha.txt" "%WORK%\ls.out" > nul || set "RC=1"
findstr /c:"beta.txt" "%WORK%\ls.out" > nul || set "RC=1"
if not "%RC%"=="0" goto :done
echo [PASS] ls listed created files
