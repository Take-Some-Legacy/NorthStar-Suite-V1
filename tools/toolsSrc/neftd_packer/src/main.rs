fn main() {
    ...
    let telemetry = northstar_cli::ulog::ToolRunInstrumentation::start("northstar-neftd-packer", command, &raw_args);
    let args = northstar_cli::ulog::strip_ulog_args(raw_args);
    match northstar_neftd_packer::commands::dispatch(args) {
