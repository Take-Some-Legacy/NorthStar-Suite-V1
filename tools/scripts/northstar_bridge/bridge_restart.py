...
def bridge_restart(ctx: BridgeContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    This intentionally delegates only to tools/scripts/ai_bridge_restart.py and
    only for its allow-listed commands. The helper itself refuses to kill
    ...
    helper = ctx.root / "tools" / "scripts" / "ai_bridge_restart.py"
    if not helper.exists():
        raise BridgeError("AI bridge restart helper is missing", "restart_helper_missing", {"path": rel(ctx.root, helper)})
    ...
    cmd: List[str] = [*ctx.python_cmd, str(helper), command, "--root", str(ctx.root), "--host", host, "--port", str(port), "--timeout", str(timeout), "--wait-sec", str(wait_sec)]
    if bool(args.get("dry_run", False)):
        cmd.append("--dry-run")
    emit("BRIDGE", "restart helper", command=command, host=host, port=port, dry_run=bool(args.get("dry_run", False)))
    started = time.time()
    proc = subprocess.run(cmd, cwd=str(ctx.root), text=True, encoding="utf-8", errors="replace", stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=False, timeout=max(5, int(wait_sec) + 10))
...
def bridge_restart_sequence(ctx: BridgeContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"ok": ok, "steps": steps, "note": "Start aiBridge.bat http or aiBridgeServer.bat after stop-origin; wait-up can verify the new origin."}
