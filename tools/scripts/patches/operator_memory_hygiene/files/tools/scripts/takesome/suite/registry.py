...
def _clean_workspace_full(root: Path) -> int:
    return clean_workspace(root, _ns(full=True, keep_logs=False, keep_cache=False, keep_plugin_binaries=False, target_scope=""))
...
def build_builtin_registry() -> SuiteRegistry:
    ...
    maintenance = (
        ...
        SuiteAction("build.plugins.force_stale", "Force rebuild stale plugins", "force rebuild stale plugin/codec targets for active profile/platform", force_rebuild_stale_plugins, "BUILD", "maintenance", "plugins", "force_rebuild", "active"),
    ...
    cleaning = (
        ...
        SuiteAction("workspace.clean.full", "Full workspace cleanup", "targets + generated logs/cache/build-state/runtime binaries", _clean_workspace_full, "CLEAN", "cleaning", "workspace", "destructive_cleanup"),
    ...
    base_actions = (*profile, *maintenance, *building, *patching, *cleaning, *git, *devtools)
