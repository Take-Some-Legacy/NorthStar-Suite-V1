...
def _command_runner(command: str, args: tuple[str, ...]):
    def run(root: Path) -> int:
        if not command:
            ...
            return 2
        # Descriptor commands are canonical NOESIS/Suite CLI commands.
        # Do not prefix them with an extra `suite` token: `python -m noesis`
        # already routes direct Suite commands such as `suite-actions-validate`,
        # `tools-validate`, `noesis-test-dev-repo`, and `tools`.
        cmd = [sys.executable, "-m", "noesis", command, *args]
