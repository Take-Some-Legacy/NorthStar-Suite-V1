...
from .noesis_audit_log import atomic_write_json, atomic_write_text, emit
...
try:
    from .noesis_task_completion import should_stop_for_completion, update_task_completion_state
except Exception:  # pragma: no cover
    should_stop_for_completion = None
...
def _write_loop_owner(root: Path, *, role: str, cycle_index: int, args: argparse.Namespace, state_dir: Path) -> None:
    ...
    lock_path = lock_dir / OWNER_LOCK_FILE
    atomic_write_json(lock_path, payload, kind="owner-lock", cycle=cycle_index, task=role, action="write-owner-lock")
    emit("LOCK_ACQUIRE", path=str(lock_path), utc=payload["updated_utc"], pid=os.getpid(), owner=role, cycle=cycle_index, task=role)
...
def run_intelligence_cycle(*, root: Path, cycle_index: int, goal: str, top: int, model: str, openai_allowed: bool, local_model_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "next_command": f"python python -m noesis suite suite --run {selected[0].action_id}" if selected else "",
    }
...
def write_pilot_status(root: Path, status_path: Path, cycle: dict[str, Any]) -> None:
    ...
    atomic_write_text(status_path, "\n".join(lines), kind="pilot-status", cycle=cycle.get("cycle"), task=str(((cycle.get("assigned_task") or {}).get("task") or {}).get("id") or ""), action="write-pilot-status")
...
def write_operator_request(root: Path, request_path: Path, cycle: dict[str, Any]) -> None:
    ...
    atomic_write_text(request_path, "\n".join(header) + body + "\n", kind="operator-request", cycle=cycle.get("cycle"), task=str(((cycle.get("assigned_task") or {}).get("task") or {}).get("id") or ""), action="write-operator-request")
