...
from .intelligence.descriptor_paths import suite_action_descriptor_path, suite_action_descriptor_relpath
...
def run_self_checks(root: Path, *, registry_actions: list[str], torch_status: TorchStatus) -> list[dict[str, object]]:
    ...
    cli_path = root / "noesis" / "suite" / "cli.py"
    checks.append(_check("NOESIS suite CLI exists", cli_path.exists(), rel(root, cli_path)))
    checks.append(_check("suite-intelligence registry hook registered", "suite-intelligence" in _read_text(root / "noesis" / "suite" / "commands" / "cli_hooks.py"), "registry command hook should exist"))
    checks.append(_check("suite action descriptor exists", suite_action_descriptor_path(root, "suite", "suite.intelligence.analyze").exists(), suite_action_descriptor_relpath("suite", "suite.intelligence.analyze")))
