...
def _run_engine_script(repo_root: Path, script_name: str, args: list[str], *, log: TeeLog | None = None) -> int:
    ...
    script = engine_core_root(repo_root) / "scripts" / script_name
    if not script.exists():
        own_log.emit(f"[ERROR] Engine scanner missing: {rel(repo_root, script)}")
        return 1
    cmd = [sys.executable, str(script), *args]
    own_log.emit("[CMD] " + " ".join(cmd))
    completed = subprocess.run(cmd, cwd=engine_core_root(repo_root), text=True, capture_output=True)
    if completed.stdout:
...
def run_p0_invariant_scan(repo_root: Path, *, strict_large_files: bool = False, strict_boundaries: bool = False, fail_tracked_large_debt: bool = False, log: TeeLog | None = None) -> int:
    args: list[str] = []
...
def run_p1_capability_conformance_scan(repo_root: Path, *, log: TeeLog | None = None) -> int:
    return _run_engine_script(repo_root, "p1_capability_conformance_scan.py", [], log=log)


def run_p2_schema_property_scan(repo_root: Path, *, log: TeeLog | None = None) -> int:
    return _run_engine_script(repo_root, "p2_schema_property_scan.py", [], log=log)


def run_p21_schema_runtime_scan(repo_root: Path, *, log: TeeLog | None = None) -> int:
    return _run_engine_script(repo_root, "p21_schema_runtime_scan.py", [], log=log)


def run_p3_editor_shell_scan(repo_root: Path, *, log: TeeLog | None = None) -> int:
    return _run_engine_script(repo_root, "p3_editor_shell_scan.py", [], log=log)


def run_p4_import_pipeline_scan(repo_root: Path, *, log: TeeLog | None = None) -> int:
    return _run_engine_script(repo_root, "p4_import_pipeline_scan.py", [], log=log)


def run_p5_world_scene_save_load_scan(repo_root: Path, *, log: TeeLog | None = None) -> int:
    return _run_engine_script(repo_root, "p5_world_scene_save_load_scan.py", [], log=log)


def run_p6_gameplay_foundation_scan(repo_root: Path, *, log: TeeLog | None = None) -> int:
    return _run_engine_script(repo_root, "p6_gameplay_foundation_scan.py", [], log=log)


def run_p7_rendering_maturity_scan(repo_root: Path, *, log: TeeLog | None = None) -> int:
    return _run_engine_script(repo_root, "p7_rendering_maturity_scan.py", [], log=log)


def run_p8_reference_module_completeness_scan(repo_root: Path, *, strict_reference_parity: bool = False, log: TeeLog | None = None) -> int:
    args: list[str] = []
...
def run_dataset_maturity_scan(repo_root: Path, *, write_index: bool = True, strict: bool = False, log: TeeLog | None = None) -> int:
    args: list[str] = []
    if write_index:
        args.append("--write")
    if strict:
