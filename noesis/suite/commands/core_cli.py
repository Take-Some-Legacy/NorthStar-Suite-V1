...
def register_core_parsers(sub: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    p = sub.add_parser("noesis-test-dev-repo", help="Run NOESIS testDevRepo verification workflow.")
    p.add_argument("mode", nargs="?", default="verify", choices=["verify"])
    p.add_argument("--run-id", default="")
    p.add_argument("--scope", default="noesis-core", choices=["noesis-core", "full-repo"])
    p.add_argument("--apply-current-diff", action="store_true", default=True)
    p.add_argument("--no-apply-current-diff", dest="apply_current_diff", action="store_false")
...
def dispatch_core_command(command: str, root: Path, ns: argparse.Namespace) -> int | None:
    ...
    if command == "noesis-test-dev-repo":
        args = [getattr(ns, "mode", "verify") or "verify"]
        if getattr(ns, "run_id", ""):
            args.extend(["--run-id", ns.run_id])
        args.extend(["--scope", getattr(ns, "scope", "noesis-core") or "noesis-core"])
        if bool(getattr(ns, "apply_current_diff", True)):
            args.append("--apply-current-diff")
        else:
            args.append("--no-apply-current-diff")
        return noesis_test_dev_repo_main(args, source_root=root)
    return None
