...
from .noesis_audit_log import atomic_write_json, atomic_write_text, append_jsonl
...
def write_json_atomic(path: Path, payload: dict[str, Any]) -> None:
    atomic_write_json(path, payload, kind="workloop-json", cycle=payload.get("cycle"), task=str(payload.get("assigned_task_id") or payload.get("selected_action_id") or ""), action="write-workloop-json")
...
def append_workloop_trace(
    ...
    *,
    ...
    append_jsonl(trace_path, event, kind="workloop-trace-jsonl", cycle=event.get("cycle"), task=str(event.get("assigned_task_id") or event.get("selected_action_id") or ""), action="append-workloop-trace")
