...
def run_suite_action(repo: Path, action_id: str, timeout: int = 240) -> CommandResult:
    # Use the canonical module entrypoint inside testDevRepo. Batch files are
    # Windows launchers and are not a registration dependency.
...
def run_tests(repo: Path, proof: ProofLog) -> Phase:
    proof.emit("TEST", command="suite.actions.validate+tools.validate", status="running")
    commands = [
    ...
    proof.emit("TEST", command="suite.actions.validate+tools.validate", status=status, passed=len(commands) - len(failed), failed=len(failed))
    return Phase(
        name="tests",
        status=status,
...
def changed_python_files(change_data: dict[str, Any]) -> list[str]:
    out: list[str] = []
...
def run_build(repo: Path, change_data: dict[str, Any], proof: ProofLog) -> Phase:
    py_files = changed_python_files(change_data)
    ...
    if py_files:
        ...
        # In offline snapshot mode there is no .git and no changed-file list.
        # Validate the canonical runtime package instead of probing Git.
        commands.append(run_cmd([sys.executable, "-m", "compileall", "-q", "noesis"], cwd=repo, timeout=180))
    failed = [c for c in commands if not c.ok]
    status = "ok" if not failed else "failed"
...
def verify_command(source_root: Path, args: argparse.Namespace) -> int:
    ...
    if args.scope == "full-repo":
        proof.emit("VERIFY", kind="full-repo-gate", status="rejected", reason="full_repo_gate_not_implemented")
        return finalize(run_id, run_dir, workspace_repo, phases, proof, status=FINAL_REJECTED, reason="full_repo_gate_not_implemented", scope=args.scope)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="noesis-test-dev-repo", description="NOESIS verified testDevRepo workflow runner.")
