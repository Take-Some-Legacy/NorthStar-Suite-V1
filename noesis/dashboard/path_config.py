...
from .runs import paths_payload, read_json
...
def update_dashboard_paths(root: Path, updates: dict[str, Any]) -> dict[str, Any]:
    ...
    if not accepted:
        return {"ok": False, "error": "no_editable_paths_supplied", "rejected": rejected, "paths": paths_payload(root)}
    ...
    return {
        "ok": True,
        "schema": "noesis.dashboard.paths.update.v1",
