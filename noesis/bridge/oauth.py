...
def protected_resource_metadata(base: str, resource_path: str = "/mcp") -> dict[str, Any]:
    ...
    return {
        ...
        "short_title": release_info.BRIDGE_SHORT_TITLE,
        "name": release_info.BRIDGE_SERVICE_NAME,
        "description": release_info.BRIDGE_PUBLIC_DESCRIPTION,
        "service_documentation": base.rstrip("/") + "/health",
        "service_icon": base.rstrip("/") + release_info.BRIDGE_DEFAULT_ICON,
        "product": release_info.BRIDGE_PRODUCT_NAME,
        "vendor": release_info.BRIDGE_VENDOR,
        "runtime_layout": release_info.BRIDGE_RUNTIME_LAYOUT,
...
def authorize(root: Path, headers: Any, query: str) -> tuple[int, dict[str, str], bytes]:
    ...
    if response_type != "code" or not client_id or not redirect_uri:
        title = html.escape(release_info.html_title("OAuth authorize error"))
        body = (
            "<!doctype html><html><head>"
            f"<title>{title}</title>"
            "<meta charset=\"utf-8\">"
            f"<meta name=\"application-name\" content={json.dumps(release_info.BRIDGE_PUBLIC_TITLE)}>"
            "</head><body>"
            f"<h1>{html.escape(release_info.BRIDGE_PUBLIC_TITLE)}</h1>"
            "<p>Invalid OAuth authorize request.</p>"
            "</body></html>"
        ).encode("utf-8")
        return 400, {"Content-Type": "text/html; charset=utf-8", "X-Noesis-Title": release_info.BRIDGE_PUBLIC_TITLE}, body
    ...
    title = html.escape(release_info.html_title("OAuth authorized"))
    body = (
        "<!doctype html><html><head>"
        f"<title>{title}</title>"
        "<meta charset=\"utf-8\">"
        f"<meta name=\"application-name\" content={json.dumps(release_info.BRIDGE_PUBLIC_TITLE)}>"
        f"<meta name=\"description\" content={json.dumps(release_info.BRIDGE_PUBLIC_DESCRIPTION)}>"
        f"<script>location.href={json.dumps(location)};</script>"
        "</head><body>"
        f"<h1>{html.escape(release_info.BRIDGE_PUBLIC_TITLE)}</h1>"
        "<p>NOESIS Suite connection authorized.</p>"
        f"<p>Redirecting to <code>{html.escape(redirect_uri)}</code>.</p>"
        f"<a href={json.dumps(location)}>Continue</a>"
        "</body></html>"
    ).encode("utf-8")
    return 302, {"Location": location, "Content-Type": "text/html; charset=utf-8", "X-Noesis-Title": release_info.BRIDGE_PUBLIC_TITLE}, body
