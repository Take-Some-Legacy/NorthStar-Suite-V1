...
from .http_mounts import HTTP_MOUNTS, dashboard_paths, operator_get_paths, public_web_paths
...
class McpRouteProfile:
    ...
    The bridge HTTP server is transport. MCP paths are one mounted surface over
    the common web contract. Dashboard paths are another surface and therefore
    are classified here without embedding dashboard business logic here.
    """

    endpoint: str = "/mcp"
    discovery_paths: tuple[str, ...] = ("/", "/mcp")
    mcp_paths: tuple[str, ...] = ("/mcp",)
    public_get_paths: tuple[str, ...] = ("/", "/mcp", HTTP_MOUNTS.health, HTTP_MOUNTS.favicon, *dashboard_paths())
...
def load_mcp_route_profile(root: Path, operator_root: Path | None = None) -> McpRouteProfile:
    """Load MCP route policy from config/env while keeping web mounts shared."""
    ...
    public_get_paths = unique_http_paths(data.get("public_get_paths") or (*discovery_paths, *public_web_paths()))
    for required in public_web_paths():
        if required not in public_get_paths:
            public_get_paths = (*public_get_paths, required)
...
def is_dashboard_path(path: str) -> bool:
    return path in dashboard_paths() or path.startswith(HTTP_MOUNTS.dashboard_static + "/")
