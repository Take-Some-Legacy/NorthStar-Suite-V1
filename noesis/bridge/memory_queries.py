...
from .contracts import BRIDGE_VERSION, BridgeContext, now_utc
...
def _suite_version_from_canonical_tree(ctx: BridgeContext) -> tuple[str, str | None]:
    candidates = [
        ctx.root / "noesis" / "suite" / "suite" / "shell.py",
        ctx.root / "noesis" / "suite" / "shell.py",
    ]
    ...
    for path in candidates:
        ...
        if marker in text:
            return text.split(marker, 1)[1].split('"', 1)[0], rel(ctx.root, path)
        return "unknown", rel(ctx.root, path)
    return "unknown", None
...
def collect_current_state(ctx: BridgeContext, args: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    suite_version, suite_shell_rel = _suite_version_from_canonical_tree(ctx)
    ...
    return {
        ...
        "suite": {"version": suite_version, "shell": suite_shell_rel},
        "workspace": {
            "root": str(ctx.root),
            "markers": {
                ...
                "noesis/suite/cli.py": (ctx.root / "noesis" / "suite" / "cli.py").exists(),
                "noesis/bridge/cli.py": (ctx.root / "noesis" / "bridge" / "cli.py").exists(),
