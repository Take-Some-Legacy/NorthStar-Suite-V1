...
CONFIG_ROOT_REL = Path("config") / "noesis"
...
class ConfigLoadResult:
    ...
    The canonical config root is config/noesis.  Runtime state remains under
    machine-local Suite state roots; source-controlled configuration does not
    fall back to historical duplicate trees.
    """
...
def config_candidates(
    ...
    *,
    ...
    `include_legacy` is accepted for API stability but intentionally ignored:
    NOESIS no longer falls back to duplicate historical config roots.
    """
    rel = Path(filename)
    primary = [
...
def load_config_json(
    ...
    *,
    ...
    for path in config_candidates(root, filename, operator_root=operator_root, include_legacy=include_legacy):
        data = read_json_object(path)
        if data:
            return ConfigLoadResult(data, path, "canonical")
    return ConfigLoadResult({}, source="defaults")
